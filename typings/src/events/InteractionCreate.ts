...
export default class AloeInteraction extends AloeEvent {
  [x: string]: any;
    ...
    this.run = async (interaction: any) => {
      if (interaction.isButton()) {
        ...
        if (interaction.customId === `avon_skip`) {
          if (!this.client.api.get(interaction.guildId).data.get(`Aloe`))
            return await interaction.message.delete().catch(() => { });
          if (
            interaction.message.id !==
            this.client.api.get(interaction.guild.id).data.get("Aloe").id
          )
            return await interaction.message.delete().catch(() => { });
          if (
            !interaction.member.voice.channel &&
