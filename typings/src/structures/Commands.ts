...
export default class AloeCommands {
  client: any;
  loaded: boolean;
    ...
    fs.readdirSync(`./build/src/commands/`).forEach((dir) => {
      fs.readdirSync(`./build/src/commands/${dir}`)
        ...
        .forEach(async (file) => {
          ...
          let AloeCommand = new command(this.client);
          this.messages.set(AloeCommand.name, AloeCommand);
          this.client.logger.debug(
            `Command Loaded: ${AloeCommand.name} loaded!`
          );
        });
    });
